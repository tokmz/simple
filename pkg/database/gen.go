...
func (g *Generator) SetupModelRelations() {
	...
	userOpts := []gen.ModelOpt{
		...
		gen.FieldNew("Department", "*Department", field.Tag{
			"json": "department",
			"gorm": "foreignKey:DepartmentID;references:ID",
		}),
		// 用户与岗位的一对一关系 - 使用指针类型
		...
		// 用户与角色的多对多关系 - 使用指针数组类型
		gen.FieldNew("Roles", "[]*Role", field.Tag{
			"json": "roles",
			"gorm": fmt.Sprintf("many2many:%s;foreignKey:ID;joinForeignKey:UserID;references:ID;joinReferences:RoleID", userRoleTable),
		}),
	}
	...
	roleOpts := []gen.ModelOpt{
		softDeleteField,
		// 角色与用户的多对多关系 - 使用指针数组类型
		gen.FieldNew("Users", "[]*User", field.Tag{
			"json": "users",
			"gorm": fmt.Sprintf("many2many:%s;foreignKey:ID;joinForeignKey:RoleID;references:ID;joinReferences:UserID", userRoleTable),
		}),
	}
	...
	// 应用基本模型
	g.Gen.ApplyBasic(user, dept, pos, role, menu)

	// 应用关联表模型
	g.Gen.ApplyBasic(
		g.Gen.GenerateModel(userRoleTable),
	)
}
